...
//LINE<JIRA_ID><DATE20130606><BUG_INFO>zenghaihui
/*************************************************
* BQ25158 mtk doc for R -sense 68, 9201 9301 9311 use 51
* R -sense 51   200mA-->266mA   100mA-->133mA
*************************************************/
...
#define bq24158_AC_CHARGING_CURRENT_750 //wangyanhui add 
...
#if (BAT_NTC_10 == 1)
//LINE<JIRA_ID><DATE20130719><add 65 70>zenghaihui
    BATT_TEMPERATURE Batt_Temperature_Table[] = {
        {-20,68237},
        {-15,53650},
        {-10,42506},
    ...
    else if(Res<=Batt_Temperature_Table[18/*16*/].TemperatureR) //LINE<JIRA_ID><DATE20130719><BUG_INFO>zenghaihui
    {
        #if 0
    ...
    else
    {
        ...
        for(i=0;i<=18/*16*/;i++) //LINE<JIRA_ID><DATE20130719><BUG_INFO>zenghaihui
        {
    ...
    if(temp_init_flag==0)
    {
        if(g_enable_high_vbat_spec == 1)
        {
            if(g_pmic_cid == 0x1020)
                ...
                bq24158_config_interface_reg(0x06,0x78); // haihui.zeng 0x77
        }
        else
            bq24158_config_interface_reg(0x06,0x70);
        ...
        if(g_enable_high_vbat_spec == 1)
        {
            if(g_pmic_cid == 0x1020)
                ...
                bq24158_config_interface_reg(0x02,0xae); // haihui.zeng 0xaa
        }
        else
            bq24158_config_interface_reg(0x02,0x8e);
...
void bq24158_set_ac_current(void)
{
    ...
    if(g_enable_high_vbat_spec == 1)
    {
        if(g_pmic_cid == 0x1020)
            ...
            bq24158_config_interface_reg(0x06,0x78); //set ISAFE // haihui.zeng 0x77
    }
    else
        bq24158_config_interface_reg(0x06,0x70); //set ISAFE
...
#else        
            //LINE<JIRA_ID><DATE20130824><high bat>zenghaihui
            if((BMT_status.temperature < MIN_NORMAL_CHARGE_TEMPERATURE) || (BMT_status.temperature > MAX_NORMAL_CHARGE_TEMPERATURE))
            {
                bq24158_config_interface_reg(0x01,0x78);
                
                //LINE<JIRA_ID><DATE20131009><high bat, 4v CV>zenghaihui
                if(BMT_status.temperature > MAX_NORMAL_CHARGE_TEMPERATURE)
                {
                    bq24158_config_interface_reg(0x02,0x66); //for 4.0v CV threshold
                }
                else
                {
                    bq24158_config_interface_reg(0x02,0xae); // haihui.zeng 0xaa
                ...
                if(g_low_power_ready == 1)
                    bq24158_config_interface_reg(0x04,0x19); // 133mA
                else
                    bq24158_config_interface_reg(0x04,0x1B); // 266mA
            }
            else
            {
...
int BAT_CheckBatteryStatus_bq24158(void)
{
    ...
    //LINE<JIRA_ID><DATE20131019><high bat, 4v CV>zenghaihui
    if ((BMT_status.temperature < 0)
            ||(BMT_status.temperature > 60)
            ||(BMT_status.temperature > MAX_NORMAL_CHARGE_TEMPERATURE && BMT_status.bat_vol > 4050)
        )
    {
        if (Enable_BATDRV_LOG >= 1) {
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY] Charging out of condition:  BMT_status.temperature = %d, BMT_status.bat_vol  = %d\r\n", 
    ...
    //LINE<JIRA_ID><DATE20130729><remove warning dialog while re-charge>zenghaihui
    #if defined(BATTERY_NOTIFY_CASE_0002)
        printk(  "[BATTERY] zenghaihui Battery re-charge, reset notify status \n\r");                
...
{    
    ...
    else
    {
        ...
        if( bq24158_status == 0x2 )
        {
            ...
            {
                //LINE<JIRA_ID><DATE20131018><high bat, 4v CV>zenghaihui
                if(BMT_status.temperature > MAX_NORMAL_CHARGE_TEMPERATURE)
                {
                    printk("[BATTERY:bq24158] Battery disable charging while in high temperature (%d) \n", BMT_status.temperature); 
